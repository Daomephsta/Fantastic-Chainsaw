...
public class MetadataHandler 
{
	private static LoadingCache<IJavaProject, ProjectMetadata> projectToMetadata = CacheBuilder.newBuilder().maximumSize(25).initialCapacity(10).
			removalListener(new RemovalListener<IJavaProject, ProjectMetadata>() 
			{
				@Override
			...
			.build(new CacheLoader<IJavaProject, ProjectMetadata>() 
			{
				...
				{	
					...
					try 
					{
						...
						{
							System.out.println("Loading metadata for " + project.getElementName() + " from file");
							metadata = ProjectMetadata.createFromFile(metadataFile);
						}
						else
						{
							System.out.println("Fetching metadata for " + project.getElementName() + " from build.gradle");
							//If it doesn't, load the metadata from the build.gradle, create the metadata file and write the metadata to the file
	...
	{
		private static final Gson METADATA_SERIALISER = new GsonBuilder().registerTypeAdapter(Version.class, new Version.Serialiser()).setPrettyPrinting().create();
